/**
 * Amélioration progressive du formulaire de dépôt d'avis.
 *
 * Le formulaire fonctionne déjà sans ce fichier : c'est un `<form method="post">`
 * natif qui poste sur /api/avis. Ce script ajoute seulement :
 *   - l'horodatage d'ouverture (filtre anti-robot côté serveur) ;
 *   - l'envoi sans rechargement, pour qu'une erreur ne vide pas l'avis déjà
 *     rédigé — perdre un texte de 1 000 caractères, c'est perdre l'avis ;
 *   - l'affichage du message d'erreur sous le bouton, sans refermer le bloc.
 *
...
(function () {
  ...
  var form = document.querySelector('[data-formulaire-avis]');
  if (!form) return;
  ...
  form.addEventListener('submit', function (evenement) {
    ...
    fetch(form.action, {
      ...
      .then(function (reponse) {
        return reponse
          ...
          .then(function (donnees) {
            if (reponse.ok) {
              window.location.assign('/avis/merci/');
              return;
            }
      ...
      .catch(function (erreur) {
        afficherErreur(
          erreur.message + ' Vous pouvez aussi nous écrire à contact@aimezlanature.fr.'
        );
        if (bouton) {
