...
| E-commerce | `/api/checkout` sans contrôle d'origine | À corriger | Un POST **sans en-tête `Origin`** est accepté (200 + session Stripe créée). N'importe qui peut créer des sessions en masse → coûts et limites d'API Stripe. Aucune donnée exposée, mais à limiter. La branche `rate-limiting-a-verifier` existe déjà : la finir. | P1 |
